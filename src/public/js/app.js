    ...
    _createClass(Signin, [{
        ...
        value: function formSubmit() {
            ...
            form.addEventListener("submit", function (e) {
                ...
                _this2.request(opts, function (error, resp, data) {
...
var Tasks = function (_NTask) {
    ...
    _createClass(Tasks, [{
        ...
        value: function renderTaskList() {
            ...
            this.request(opts, function (error, resp, tasks) {
                if (error) {
                    _this2.emit("error", error);
                } else {
                    _this2.body.innerHTML = _tasks2.default.render(tasks);
        ...
        value: function taskDoneCheckbox() {
            ...
            for (var i = 0; i < dones.length; i++) {
                dones[i].addEventListener("click", function (e) {
                    ...
                    _this3.request(opts, function (error, resp, data) {
                        if (error || resp.status === 412) {
                            _this3.emit("update-error", error);
                        } else {
                            _this3.emit("update");
                        }
                    });
...
var renderTasks = function renderTasks(tasks) {
    return tasks.map(function (task) {
        var done = task.done ? "ios-checkmard" : "ios-circle-outline";
